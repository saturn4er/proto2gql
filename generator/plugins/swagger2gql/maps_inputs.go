...
func (g *Plugin) inputMapGraphQLName(mapFile *parsedFile, res *parser.Map) string {
	return g.inputMessageVariable(mapFile, res.Message) + "__" + camelCase(res.Field.Name)
}

func (g *Plugin) inputMapVariable(mapFile *parsedFile, res *parser.Map) string {
	return g.inputMessageVariable(mapFile, res.Message) + "__" + camelCase(res.Field.Name)
}

func (g *Plugin) fileMapInputObjects(file *parsedFile) ([]graphql.MapInputObject, error) {
